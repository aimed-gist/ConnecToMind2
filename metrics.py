...
def two_way_identification(args, recons, gts, model, preprocess, feature_layer=None):
    ...
    # MindBridge 방식: column-wise (axis=0)
    # r[i,j] = correlation between gt[i] and pred[j]
    # congruents[j] = r[j,j] = correlation between gt[j] and pred[j] (diagonal)
    # success[i,j] = True if r[i,j] < congruents[j] (i.e., pred[j] is more similar to gt[j] than to gt[i])
    # success_cnt[j] = number of gt images that pred[j] is less similar to than gt[j]
    r = np.corrcoef(gt_feats, pred_feats)
    r = r[:len(gt_feats), len(gt_feats):]
    congruents = np.diag(r)
    success = r < congruents
    success_cnt = np.sum(success, 0)  # axis=0: column-wise
    return np.mean(success_cnt) / (len(gt_feats) - 1)
...
def get_metric(args):
    """
    Metric 모델들을 로드하여 딕셔너리로 반환

    Returns:
        metrics: dict containing metric functions and models
    """
    device = args.device
    ...
    # CLIP (MindBridge 방식: CenterCrop 없이 전체 이미지 사용)
